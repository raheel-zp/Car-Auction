...
function EventsPage({allEvents,Events,setEvents,EventsLoading,setEventsLoading}) {
    ...
    const submitform = async (event)=>{
        ...
        const res = await fetch(`${url}UploadEvents`, {
        method: 'POST',
        body: formData
        });
        ...
        <div className='flex_wrap' style={{position:'relative'}} >
            {
                Events.map((val,ind)=>{
                    return(<>
                        <EventsBox val={val} allEvents={allEvents} ind={ind} />
                    </>)
                })
            }
